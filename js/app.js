...
const comment = (() => {
    ...
    const kirim = async () => {
        ...
        if (nama.length == 0) {
            alert('tên không được để trống đâu ní ơi :v');
            return;
        }

        if (nama.length >= 25) {
            alert('Độ dài tên tối đa là 25 nha ní ');
            return;
        }

        if (hadir == 0) {
            alert('vui lòng chọn quan hệ với tôi :))');
            return;
        }

        if (komentar.length == 0) {
            alert('rồi nhắn tin mà để trống v là nhắn dữ chưa =))');
            return;
        }
    ...
    const innerComment = (data) => {
        ...
        <div class="d-flex flex-wrap justify-content-between align-items-center">
            <div class="d-flex flex-wrap justify-content-start align-items-center">
                <button style="font-size: 0.8rem;" onclick="comment.balasan(this)" data-uuid="${data.uuid}" class="btn btn-sm btn-outline-dark rounded-3 py-0">Trả lời</button>
                ${owns.has(data.uuid)
                ? `
                <button style="font-size: 0.8rem;" onclick="comment.edit(this)" data-uuid="${data.uuid}" class="btn btn-sm btn-outline-dark rounded-3 py-0 ms-1">Sửa</button>
    ...
    const balas = async () => {
        ...
        if (nama.length == 0) {
            alert('tên không được để trống nha ba');
            return;
        }

        if (nama.length >= 25) {
            alert('nhớ tên là >= 25 ký tự');
            return;
        }

        if (komentar.length == 0) {
            alert('rồi ai mượn để trống lời nhắn ?');
            return;
        }
    ...
    const ubah = async () => {
        ...
        if (document.getElementById(id).getAttribute('data-parent') === 'true' && hadir == 0) {
            alert('chọn các chức vụ đi bà ');
            return;
        }

        if (komentar.length == 0) {
            alert('không có để trống =_)');
            return;
        }
    ...
    const hapus = async (button) => {
        if (!confirm('Chắc chắn là xóa đúng không')) {
            return;
        }
